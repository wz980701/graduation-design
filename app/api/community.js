...
router.post('/update', async (ctx) => { // 更新社团信息 已测试
    await CommunityDao.update(ctx.request.body);
...
router.get('/getUsers', async (ctx) => { // 获取社团成员列表
    const data = await CommunityDao.getUsers(ctx.request.query);
    ctx.body = res.json(data, '获取成员列表成功');
});

router.get('/getApplyList', async (ctx) => { // 获取申请加入列表
    const data = await CommunityDao.getApplyList(ctx.request.query);
    ctx.body = res.json(data, '获取申请列表成功');
});

router.post('/join', auth, async (ctx) => { // 用户申请加入社团
    await CommunityDao.join({...ctx.request.body, userId: ctx.state.userId});
    ctx.body = res.success('申请成功');
});

router.get('/passUser', async (ctx) => { // 用户申请通过
...
router.post('/updateAnnounce', async (ctx) => { // 更新公告内容 已测试
    await CommunityAnnounceDao.updateAnnounce(ctx.request.body);
    ctx.body = res.success('更新公告成功');
});

router.get('/deleteAnnounce', async (ctx) => { // 删除公告 已测试
    const { id } = ctx.request.query;
...
router.get('/removeUser', auth, async (ctx) => { // 删除用户或者用户退出社团
    await CommunityDao.removeUser({...ctx.request.query, userId: ctx.state.userId});
    ctx.body = res.success('删除成功');
});

router.get('/userCommunityList', auth, async (ctx) => { // 用户获取加入的社团列表
...
router.get('/getSelectCommunityList', auth, async (ctx) => { // 获取用户的社团列表
